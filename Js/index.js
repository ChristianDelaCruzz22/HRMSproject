...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function handleUserStatus(user) {
        try {
            ...
            if (dbError || !appData) {
                showToast("Account found, but application details are missing.", "info");
            ...
            if (appData.status === 'Approved') {
                ...
                showToast(`Access Denied: Your application is currently ${appData.status || 'Pending'}.`, "info");
            }

        } catch (err) {
            console.error("Status Check Error:", err);
            showToast("Error verifying account status.", "error");
        }
    }
});
