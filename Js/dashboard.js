...
async function renderStats(role, userId) {
    ...
    if (role === 'SuperAdmin') {
        const { count: emps } = await _supabase.from('employee').select('*', { count: 'exact', head: true });
...
async function fetchRecentActivity(role, userId) {
    ...
    let query = _supabase.from('attendance').select('*, employee(first_name, last_name)').order('created_at', { ascending: false }).limit(5);
    ...
    body.innerHTML = (!data || data.length === 0) ? 
        ...
        data.map(item => `
            <tr>
                <td>${item.employee?.first_name || 'User'}</td>
                <td>Clock In</td>
